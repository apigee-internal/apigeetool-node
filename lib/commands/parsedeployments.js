...
module.exports.formatDeployment = function(d) {
  ...
  if (d.uris) {
    _.each(d.uris, function(uri) {
      s += util.format('  URI = %s\n', uri);
    });
  }
  if (d.partialResults) {
    _.each(d.partialResults, function(r) {
...
module.exports.parseDeploymentResult = function(d) {
  ...
  if (d.environment && Array.isArray(d.environment)) {
    ...
    deployed = _.find(d.environment, function(e) {
      return (e.state === 'deployed');
    });
    if (!deployed) {
      // None, so take the highest revision number
      deployed = _.max(d.environment, function(e) {
        return parseInt(e.revision);
      });
    }
  } else {
    deployed = d;
...
function addDeploymentInfo(r, deployed) {
  ...
  r.basePath = _.has(deployed, 'configuration.basePath') ? deployed.configuration.basePath : 'N/A';

  // Now figure out if there are partial failures
  _.each(deployed.server, function(s) {
...
function addProxyUri(deployment, vhost, proxyBasePath) {
  ...
  if (vhost.hostAliases && vhost.hostAliases.length > 0) {
    _.each(vhost.hostAliases, function(item) {
